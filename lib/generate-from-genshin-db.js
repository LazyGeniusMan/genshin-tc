const core = require('./core.js');
const db = require('genshin-db');
const fs = require('fs');
const csv = require('fast-csv');
const path = require('path');
const statsMap = {
    'Anemo DMG Bonus' : 'anemo_dmg',
    'ATK' : 'atk%',
    'CRIT DMG' : 'crit_dmg',
    'CRIT Rate' : 'crit_rate',
    'Cryo DMG Bonus' : 'cryo_dmg',
    'DEF' : 'def%',
    'Dendro DMG Bonus' : 'dendro_dmg',
    'Electro DMG Bonus' : 'electro_dmg',
    'Elemental Mastery' : 'em',
    'Energy Recharge' : 'er',
    'Geo DMG Bonus' : 'geo_dmg',
    'Healing Bonus' : 'healing_bonus',
    'HP' : 'hp%',
    'Hydro DMG Bonus' : 'hydro_dmg',
    'Physical DMG Bonus' : 'physical_dmg',
    'Pyro DMG Bonus' : 'pyro_dmg',
};

//Get data from genshin-db
function getData(type, query, level, ascended) {
    switch (type) {
        case 'character':
            return db.characters(query, { matchCategories: true });
            break;
        case 'character-stats':
            return db.characters(query).stats(level, ascended);
            break;
        case 'character-talents':
            return db.talents(query);
            break;    
        case 'weapon':
            return db.weapons(query, { matchCategories: true });
            break;
        case 'weapon-stats':
            return db.weapons(query).stats(level, ascended);
            break;
        default:
            break;
    }
}

//Generate All CSV files from data generated by genshin-db
function generateCSVData(type) {
    switch (type) {
        case 'character-stats':
            //List names that will be generated
            var names = getData('character', 'names');
            generateCharaStats(names, type);
            break;
        case 'character-talents':
            break;    
        case 'weapon-stats':
            //List names that will be generated
            var names = getData('weapon', 'names');
            generateWeaponStats(names, type);
            break;
        case 'weapon-passive':
            break;
        default:
            break;
    }
}

//Generate characters stats data for CSV files
function generateCharaStats(names, type) {
    //Map for rename and order CSV header
    let keysMap = {
        level: 'level',
        ascension: 'ascension',
        hp: 'hp',
        attack: 'atk',
        defense: 'def',
        substat: 'ascension_stat',
        specialized: 'ascension_value'
    };
    //Generate every characters stats data
    for (let i = 0; i < names.length; i++) {
        const charaName = names[i];
        let charaMetadata = getData('character', charaName);
        let charaStats = [];
        //Generate every level of a character stats data
        for (let level = 1; level <= 90; level++) {
            let tempStats = getData(type, charaName, level);
            //Add character ascension stat to tempStats object
            Object.assign(tempStats, {'substat' : charaMetadata.substat});
            //Change value based on statsMap
            tempStats['substat']=statsMap[tempStats.substat];
            //Rename object keys for CSV header
            tempStats = core.renameKeys(keysMap, tempStats);
            //Order object keys
            tempStats = core.orderKeys(Object.values(keysMap), tempStats);
            //Add character stats to an array
            charaStats.push(tempStats);
            //Check if we need to add stats after ascended
            if ([20, 40, 50, 60, 70, 80].includes(level)) {
                let tempStat = getData(type, charaMetadata.name, level, '+');
                Object.assign(tempStat, {'substat' : charaMetadata.substat});
                tempStat['substat']=statsMap[tempStat.substat];
                tempStat = core.renameKeys(keysMap, tempStat);
                tempStat = core.orderKeys(Object.values(keysMap), tempStat);
                charaStats.push(tempStat);
            }
        }
        //Write CSV file
        writeCSV(type, charaStats, charaMetadata);
    }
    //Generate template
    let charaMetadata = {
        element : '_elemenType',
        name    : '_characterName',
        substat : 'CRIT DMG',
    }
    let charaStats =[];
    let tempStats = getData('character-stats', 'Hu Tao', '1');
    Object.assign(tempStats, {'substat' : charaMetadata.substat});
    tempStats['substat']=statsMap[tempStats.substat];
    tempStats = core.renameKeys(keysMap, tempStats);
    tempStats = core.orderKeys(Object.values(keysMap), tempStats);
    charaStats.push(tempStats);
    writeCSV(type, charaStats, charaMetadata);
}

//Generate weapons stats data for CSV files
function generateWeaponStats(names, type) {
    //Map for rename and order CSV header
    let keysMap = {
        level: 'level',
        ascension: 'ascension',
        attack: 'base_atk',
        substat: 'substat',
        specialized: 'substat_value'
    };
    //Generate every characters stats data
    for (let i = 0; i < names.length; i++) {
        const weaponName = names[i];
        let weaponMetadata = getData('weapon', weaponName);
        let weaponStats = [];
        //Generate every level of a character stats data
        for (let level = 1; level <= 90; level++) {
            try {
                let tempStats = getData(type, weaponName, level);
                //Add character ascension stat to tempStats object
                Object.assign(tempStats, {'substat' : weaponMetadata.substat});
                //Change value based on statsMap
                tempStats['substat']=statsMap[tempStats.substat];
                //Rename object keys for CSV header
                tempStats = core.renameKeys(keysMap, tempStats);
                //Order object keys
                tempStats = core.orderKeys(Object.values(keysMap), tempStats);
                //Add character stats to an array
                weaponStats.push(tempStats);
                //Check if we need to add stats after ascended
                if ([20, 40, 50, 60, 70, 80].includes(level)) {
                    let tempStat = getData(type, weaponMetadata.name, level, '+');
                    Object.assign(tempStat, {'substat' : weaponMetadata.substat});
                    tempStat['substat']=statsMap[tempStat.substat];
                    tempStat = core.renameKeys(keysMap, tempStat);
                    tempStat = core.orderKeys(Object.values(keysMap), tempStat);
                    weaponStats.push(tempStat);
                } 
            } catch (err) {
                //console.error(err)
            }
        }
        //Write CSV file
        writeCSV(type, '', '', weaponStats, weaponMetadata);
    }
    //Generate template
    let weaponMetadata = {
        weapontype : '_weaponType',
        name    : '_weaponName',
        substat : 'CRIT DMG',
    }
    let weaponStats =[];
    let tempStats = getData('weapon-stats', 'Homa', '1');
    Object.assign(tempStats, {'substat' : weaponMetadata.substat});
    tempStats['substat']=statsMap[tempStats.substat];
    tempStats = core.renameKeys(keysMap, tempStats);
    tempStats = core.orderKeys(Object.values(keysMap), tempStats);
    weaponStats.push(tempStats);
    writeCSV(type, '', '', weaponStats, weaponMetadata);
}

//Write CSV file
function writeCSV(type, charaStats, charaMetadata, weaponStats, weaponMetadata) {
    let rootPath = path.dirname(require.main.filename);
    switch (type) {
        case 'character-stats':
            var filename = 'stats';
            var filePath = rootPath.concat('\\data\\characters\\', charaMetadata.element.toLowerCase(), '\\', charaMetadata.name.toLowerCase(),'\\', filename ,'.csv');
            //Create folder based on file path if not exists
            core.createFolder(path.dirname(filePath));
            //Clear file content if exists
            core.clearFileContent(filePath);
            //Write CSV file
            var ws = fs.createWriteStream(filePath, {flags: 'a'});
            const charaStatCsv = csv.format({ headers: true });
            charaStatCsv.pipe(ws).on('end', () => process.exit());
            for(let ele of charaStats) {
                charaStatCsv.write(ele);                    
            }
            charaStatCsv.end();
            break;
        case 'character-talents':
            break;    
        case 'weapon-stats':
            var filename = 'stats';
            var filePath = rootPath.concat('\\data\\weapons\\', weaponMetadata.weapontype.toLowerCase(), '\\', weaponMetadata.name.toLowerCase(),'\\', filename ,'.csv');
            //Create folder based on file path if not exists
            core.createFolder(path.dirname(filePath));
            //Clear file content if exists
            core.clearFileContent(filePath);
            //Write CSV file
            var ws = fs.createWriteStream(filePath, {flags: 'a'});
            const weaponStatCsv = csv.format({ headers: true });
            weaponStatCsv.pipe(ws).on('end', () => process.exit());
            for(let ele of weaponStats) {
                weaponStatCsv.write(ele);                    
            }
            break;
        case 'weapon-passive':
            break;
        default:
            break;
    }
    
}

module.exports ={
    generateCSVData : generateCSVData,
}