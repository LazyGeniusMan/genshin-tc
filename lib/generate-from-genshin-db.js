...
function generateCharaStats(names, type) {
    ...
    keyHeader.set('substat', 'ascension_stat');
    keyHeader.set('specialized', 'ascension_value');
    ...
    for (let i = 0; i < names.length; i++) {
        ...
        for (let level = 1; level <= 90; level++) {
            ...
            for (let [key, value] of keyHeader.entries()) {
                renameObjectKey(tempStats, key, value);
            }
            ...
            if ([20, 40, 50, 60, 70, 80].includes(level)) {
                ...
                for (let [key, value] of keyHeader.entries()) {
                    renameObjectKey(tempStat, key, value);
                }
    ...
    try {
        if (fs.existsSync(filePath)) {
            let options = {
              ...
              try {
                const results = findreplace.sync(options);
...
//Rename object key
function renameObjectKey(objectName, oldKey, newKey) {
    if (oldKey !== newKey) {
        Object.defineProperty(objectName, newKey,
            Object.getOwnPropertyDescriptor(objectName, oldKey));
